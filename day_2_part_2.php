...
foreach($data as $row) {
    $row = explode(':', $row);
    $game_id = ltrim($row[0], 'Game ');
    $row = explode(';', $row[1]);
    $dice_data = [];
    $red = $blue = $green = 0;
    foreach($row as $key => $dice_game) {
        $dice_game = explode(',', $dice_game);
        foreach($dice_game as $dice) {
            $dice = ltrim($dice, ' ');
            $dice = explode(' ', $dice);
            if(${$dice[1]} < $dice[0]) {
                ${$dice[1]} = $dice[0];
            }
            $dice_data[$key][$dice[1]][] = $dice[0];
        }
    }
    $power_total += $red * $blue * $green;
    $parsed_data[$game_id] = $dice_data;
}
...
foreach($parsed_data as $game_id => $game_data) {
    ...
    foreach($game_data as $dice_game) {
        $current_row_count++;
        foreach($dice_game as $color => $dice_data) {
            $sum = array_sum($dice_data);
            if($sum > $given_data[$color]) {
                 break 2;
            }
        }

        if($current_row_count == $count) {
            $game_id_total += $game_id;
        }
    }
}

echo "game_id total: $game_id_total" . PHP_EOL;
echo "power total: $power_total";
