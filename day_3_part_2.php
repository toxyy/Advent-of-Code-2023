...
$symbols = '*-%$=@#/&+';
$gear_ratio_array = [];
$symbol_array = str_split($symbols);
...
for($row = 0; $row < $rows; $row++) {
    $next_line = isset($data[$row + 1]) ? $data[$row + 1] : [];

    $line = $data[$row];
    $line = str_split($line);

    $columns = count($line);
    $current_number = $add_gear_ratio = '';
    $add_number = false;
    for($col = 0; $col < $columns; $col++) {
        $data_point = $line[$col];
        if(is_numeric($data_point)) {
            $current_number .= $data_point;
            if(!empty($prev_line)) {
                $prev_line_cells = [
                    -1 => isset($prev_line[$col - 1])
                        ? $prev_line[$col - 1]
                        : '.',
                    0  => $prev_line[$col],
                    1  => isset($prev_line[$col + 1])
            ...
            foreach($surrounding_cells as $row_offset => $check_line) {
                foreach($check_line as $col_offset => $check_data_point) {
                    if(in_array($check_data_point, $symbol_array)) {
                        $add_number = true;
                        if($check_data_point === '*') {
                            $add_gear_ratio = $row + $row_offset . ',' . $col + $col_offset;
                        }
                    }
                }
            }
            if(!is_numeric($line[$col + 1])) {
                if($add_number) {
                    ...
                    if(!empty($add_gear_ratio)) {
                        $gear_ratio_position = explode(',', $add_gear_ratio);
                        $gear_row = $gear_ratio_position[0];
                        $gear_col = $gear_ratio_position[1];
                        $gear_ratio_array[$gear_row][$gear_col][] = $current_number;
                    }
                }
            }
        } else {
...
$gear_ratio_total = 0;
foreach($gear_ratio_array as $gear_row) {
    foreach($gear_row as $gear_col) {
        if(count($gear_col) == 2) {
            $gear_ratio_total += array_product($gear_col);
        }
    }
}

echo "gear ratio total: $gear_ratio_total" . PHP_EOL;
echo "part number total: $total";
